...
# ✅ NOVO: editar nome do cliente (sem criar outro cliente)
class EditarNomeBody(BaseModel):
    nome: str
...
# =====================================================
# ✅ NOVO: EDITAR NOME DO CLIENTE (SEM CRIAR OUTRO)
# =====================================================
@app.put("/api/fila/{fila_id}/cliente/{cliente_id}/nome")
@app.put("/api/filas/{fila_id}/cliente/{cliente_id}/nome")
async def editar_nome_cliente(fila_id: int, cliente_id: int, body: EditarNomeBody):
    try:
        nome = (body.nome or "").strip()
        if not nome or len(nome) < 3:
            raise HTTPException(status_code=400, detail="Nome inválido (mínimo 3 caracteres).")
        ...
        # valida se cliente está ativo nessa fila
        cur.execute("""
        ...
        await notify_fila_update(fila_id, "CLIENTE_EDITOU_NOME", {
            "cliente_id": cliente_id,
            "fila_cliente_id": fila_cliente_id,
            "nome": nome
        })

        return {"ok": True, "cliente_id": cliente_id, "fila_cliente_id": fila_cliente_id, "nome": nome}

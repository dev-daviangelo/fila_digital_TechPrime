...
async function entrarNaFila(){
  ...
  navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    console.log("LAT CAPTURADA:", lat);
    console.log("LNG CAPTURADA:", lng);
    ...
    console.log(err);
    alert("Permita o acesso à localização para entrar na fila.");
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      console.log("Localização capturada:", lat, lng);
        ...
        const geoResp = await fetch(`${API_BASE}/api/filas/${filaId}/cliente/${clienteId}/geo`, {
  method: "POST",
  headers: {
...
const geoData = await geoResp.json().catch(() => null);

if (!geoResp.ok) {
  if (geoResp.status === 403 && geoData?.detail?.code === "FORA_DA_AREA") {
    alert(geoData.detail.message || "Você está longe da fila.");
    return;
  }

  throw new Error(
    typeof geoData?.detail === "string"
      ? geoData.detail
      : (geoData?.detail?.message || "Erro ao atualizar localização.")
  );
}
      ...
      console.log("Cliente:", lat, lng);
      console.log("Estabelecimento:", estabLat, estabLng);
